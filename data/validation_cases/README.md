...
- **Harmless-class composition changes the answer — and breadth alone is not a fix.** `pressure_low` is the 95th percentile of whatever sits in that class, so that class defines "benign". Two runs on Qwen2.5-0.5B with a layer-6 direction:
